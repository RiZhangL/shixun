...
<script type="text/javascript">
	function deleteAnn(){
			...
			temp.action = "http://localhost:8080/staffhome/document/deleteDoc";
			temp.method = "post";
	...
	function search(){
		...
		var temp = document.createElement("form");
		temp.action = "http://localhost:8080/staffhome/document/selectDocument";
		temp.method = "post";
		temp.style.display = "none";
		var opt1 = document.createElement("textarea");
		opt1.name="queryAttr";
		opt1.value=queryAttribute;
		temp.appendChild(opt1);
		var opt2 = document.createElement("textarea");
		opt2.name="content";
		opt2.value=queryText;
						...
						<td class="fftd">
							...
							<table width="100%" border="0" cellpadding="0" cellspacing="0">
								<form action="selectDocument" method="post">
									<tr>
										<td class="font3">
											<select style="height:32;" id="queryAttribute">
											  <option value ="title">文档标题</option>
											  <option value="creator_name">发布人</option>
											</select>
					...
					<tbody>
						...
						<c:forEach items="${list}" var="item" varStatus="xh">
							<tr id="data_0" align="center">
								...
								<td align="center" width="40px;"><a href="updateDoc_pre?id=${item.id}&title=${item.title}&filename=${item.filename}&remark=${item.remark}&create_date=${item.create_date}&user_id=${item.user_id}">
									<button type="button" class="btn btn-default">
										<span class="glyphicon glyphicon-pencil"></span>
									</button>
							    </td>
							    <td align="center" width="130px;"><a
									href="downloadDoc?id=${item.id}&title=${item.title}&filename=${item.filename}&remark=${item.remark}&create_date=${item.create_date}&user_id=${item.user_id}"
